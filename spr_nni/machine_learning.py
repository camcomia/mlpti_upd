...
def apply_SVR(df_test, df_train, features):
    ...
    return y_pred
##########################################################################
def main(training_folder, model):
	print ("Finding all dataset.csv...")
	print (f"{training_folder}/*/dataset.csv")
	x = glob.glob(f"{training_folder}/*/dataset.csv", recursive=True)
	print (len(x))
	df_all = pd.DataFrame()
	...
	for low_i in group_ids[::test_batch_size]:
		print (f"Round {round_num}...")
		low_i, = np.where(group_ids == low_i)
		# print (low_i)
		low_i = int(low_i[0])
		up_i = low_i + test_batch_size
	
		test_ixs = group_ids[low_i:up_i]
		train_ixs = np.setdiff1d(group_ids, test_ixs)
	
		df_test = df.loc[df["group_id"].isin(test_ixs)]
		df_train = df.loc[df["group_id"].isin(train_ixs)]
...
if __name__ == "__main__":
	...
	main(training_folder=args.ds_path, model=args.model)
