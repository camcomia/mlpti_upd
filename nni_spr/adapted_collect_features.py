...
if __name__ == '__main__':
    ...
    spr_output_prefix = args.spr_output_prefix
    # This is the file to be read AND overwritten
    summary_path = f"{spr_output_prefix}.spr_summary.csv"
    # This file is only read
    newick_path = f"{spr_output_prefix}.spr_newicks.csv"

    # --- Validate input paths ---
    if not os.path.isfile(nni_tree_path): 
        print(f"Error: Input NNI tree file not found: {nni_tree_path}")
        sys.exit(1)
    if not os.path.isfile(summary_path): 
        print(f"Error: SPR summary file not found based on prefix: {summary_path}")
        sys.exit(1)
    if not os.path.isfile(newick_path): 
        print(f"Error: SPR newick file not found based on prefix: {newick_path}")
        sys.exit(1)
