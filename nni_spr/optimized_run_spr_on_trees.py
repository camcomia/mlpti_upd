...
import argparse
import signal
from functools import partial  # Add this import at the top of the file
...
def run_spr_analysis(index_str, input_tree_path, msa_path, params_stats_file, output_dir):
    """
    ...
    """
    print(f"Starting SPR analysis for tree: {output_dir}", flush=True)
...
def run_spr_analysis_wrapper(args):
    """
    Wrapper function to unpack arguments for run_spr_analysis.
    """
    return run_spr_analysis(*args)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_trees', '-t', required=True, help='Path to the input tree file (Newick format).')
    ...
    print(f"Starting run_on_tree.py on {args.input_trees}...")

    # Determine output directory if not provided
    output_dir_main = args.output_dir
    if output_dir_main is None:
        output_dir_main = os.path.dirname(args.input_trees)
    ...
    if not os.path.isfile(args.input_trees): 
        print(f"Error: Input tree file not found: {args.input_trees}")
        sys.exit(1)
    ...
    trees = get_input_trees(args.input_trees)

    # Prepare arguments for executor.map
    indexed_trees = [(str(i), tree, args.msa, args.params_stats_file, output_dir_main) for i, tree in enumerate(trees, start=1)]

    with ProcessPoolExecutor(max_workers=4) as executor:
        results = list(executor.map(run_spr_analysis_wrapper, indexed_trees))

    print(f"Finished run_spr_analysis on {args.input_trees}.")

    # Exit with appropriate status code
    if any(result[0] is False for result in results):
        print("One or more SPR analyses failed. Check logs for details.")
        sys.exit(1)
    else:
        print("All SPR analyses completed successfully.")