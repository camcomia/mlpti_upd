...
for e in list_int: types_dict[e] = pd.Int64Dtype() # Use nullable integer type
for e in list_float: types_dict[e] = np.float32


# --- Helper Functions ---

def log_experiment(model_name, params, metrics, rfe_active=False, n_selected=None):
    """Logs experiment parameters and metrics, noting if RFE was used."""
    rfe_info = f"(RFE Active: {n_selected} features)" if rfe_active else "(RFE Inactive)"
    print(f"--- {model_name} (Default Params) {rfe_info} Experiment ---")
    if params:
        print(f"  Default Parameters Used: {params}")
    else:
        print("  Default Parameters Used: Sklearn Defaults")

    printable_metrics = {k: f"{v:.4f}" if isinstance(v, (float, np.floating)) else v
                         for k, v in metrics.items() if v is not None}
    print(f"  Metrics: {printable_metrics}")
    print("-" * (len(model_name) + len(rfe_info) + 25))
...
def split_features_label(df, target_label, feature_cols):
    ...
    try:
        # Convert features to float32 numpy array
        ...
        for col in numeric_feature_cols:
            try:
                 # Attempt conversion column by column
...
# --- Model Application Functions (apply_RFR updated for RFE) ---

def apply_RFR(df_test, df_train, target_label, feature_cols, n_features_to_select=None):
    """Fits RFR with default parameters, optionally using RFE, predicts, returns results."""
    rfe_active = n_features_to_select is not None
    rfe_info = f"(RFE Active: {n_features_to_select} features)" if rfe_active else "(RFE Inactive)"
    print(f"Applying RandomForestRegressor {rfe_info}...")

    # 1. Split data
    X_train_orig, y_train = split_features_label(df_train, target_label, feature_cols)
    X_test_orig, y_test = split_features_label(df_test, target_label, feature_cols)
    if X_train_orig is None or X_test_orig is None: return None, None, None

    # 2. Handle NaN labels (before imputation)
    train_valid_idx = ~np.isnan(y_train)
    test_valid_idx = ~np.isnan(y_test)
    X_train_orig, y_train = X_train_orig[train_valid_idx], y_train[train_valid_idx]
    X_test_orig, y_test = X_test_orig[test_valid_idx], y_test[test_valid_idx]

    # 3. Impute NaN features on the original feature sets
    X_train_imputed, X_test_imputed = impute_nans_with_train_mean(X_train_orig, X_test_orig)
    if X_train_imputed is None or X_test_imputed is None: return None, None, None

    # Check for empty data after NaN handling
    ...
    y_pred = None
    oob = None
    f_imp_mapped = None # Feature importances mapped back to original features
    fit_time = np.nan
    rfe_selector = None # To store the fitted RFE selector if used

    # --- Define Default Parameters for RFR ---
    default_params = {
        'n_estimators': 160,
        'max_features': 0.33,
        'random_state': 42,
        'n_jobs': -1,
        'oob_score': True # Calculate OOB score only if RFE is NOT active
    }

    # --- Feature Selection (RFE) and Training ---
    X_train_final = X_train_imputed
    X_test_final = X_test_imputed

    start_time = time.time()
    if rfe_active:
        print(f"  Applying RFE to select top {n_features_to_select} features...")
        # RFE requires a base estimator
        # OOB score is not useful within RFE's internal fitting process
        rfr_base_for_rfe = RandomForestRegressor(
            n_estimators=default_params['n_estimators'], # Use same n_estimators for selection consistency
            max_features=default_params['max_features'], # Use same max_features
            random_state=default_params['random_state'],
            n_jobs=default_params['n_jobs'],
            oob_score=False # OOB is not calculated during RFE fitting
        )
        # Instantiate RFE
        rfe_selector = RFE(estimator=rfr_base_for_rfe,
                           n_features_to_select=n_features_to_select,
                           step=1) # Remove one feature per iteration
        # Fit RFE on the (imputed) training data
        rfe_selector.fit(X_train_imputed, y_train)
        # Transform training and test sets to keep only selected features
        X_train_final = rfe_selector.transform(X_train_imputed)
        X_test_final = rfe_selector.transform(X_test_imputed)
        print(f"  RFE finished. Selected {X_train_final.shape[1]} features.")
        # Deactivate OOB score for the final model when RFE is used, as it's trained on transformed data
        default_params['oob_score'] = False
        oob = None # OOB score is not applicable/calculated when using RFE here

    # --- Fit Final Model ---
    # Use the same default parameters (OOB might be False if RFE was active)
    print(f"  Fitting final RFR model with parameters: {default_params}")
    final_model = RandomForestRegressor(**default_params)
    # Train on the final feature set (either original imputed or RFE-selected)
    final_model.fit(X_train_final, y_train)
    fit_time = time.time() - start_time
    print(f"  Final RFR fitted in {fit_time:.2f} seconds.")
    ...
    f_imp_raw = final_model.feature_importances_
    if rfe_active and rfe_selector is not None:
        # Map importances back to the original feature space
        f_imp_mapped = np.zeros(X_train_imputed.shape[1]) # Array of zeros with original feature count
        f_imp_mapped[rfe_selector.support_] = f_imp_raw # Place importances in selected positions
    else:
        # If RFE wasn't active, importances correspond directly to original features
        f_imp_mapped = f_imp_raw

    # Get OOB score if it was calculated (i.e., RFE was not active)
    if default_params['oob_score']:
        oob = final_model.oob_score_ if hasattr(final_model, 'oob_score_') else None

    # --- Logging ---
    metrics = { "Test MSE": mean_squared_error(y_test, y_pred),
                "R^2 Score": r2_score(y_test, y_pred),
                "OOB Score": oob, # Will be None if RFE was active
                "Fit Time (s)": fit_time }
    log_experiment("Random Forest", default_params, metrics, rfe_active, n_features_to_select)
    ...
    # Return mapped importances
    return full_y_pred, oob, f_imp_mapped


# --- Other Model Application Functions (Unchanged) ---
...
def apply_KNN(df_test, df_train, target_label, feature_cols):
    ...
    metrics = { "Test MSE": mean_squared_error(y_test, y_pred),
                "R^2 Score": r2_score(y_test, y_pred),
                "Fit Time (s)": fit_time }
    log_experiment("K Nearest Neighbors", default_params, metrics) # RFE info not relevant here
...
def apply_SVM(df_test, df_train, target_label, feature_cols):
    ...
    metrics = { "Test MSE": mean_squared_error(y_test, y_pred),
                "R^2 Score": r2_score(y_test, y_pred),
                "Fit Time (s)": fit_time }
    log_experiment("Support Vector Machine (SVR)", default_params, metrics) # RFE info not relevant
...
def apply_BayesianRidge(df_test, df_train, target_label, feature_cols):
    ...
    metrics = { "Test MSE": mean_squared_error(y_test, y_pred),
                "R^2 Score": r2_score(y_test, y_pred),
                "Fit Time (s)": fit_time }
    log_experiment("Bayesian Ridge", default_params if default_params else "Sklearn Defaults", metrics) # RFE irrelevant
...
def apply_Lasso(df_test, df_train, target_label, feature_cols):
    ...
    metrics = { "Test MSE": mean_squared_error(y_test, y_pred),
                "R^2 Score": r2_score(y_test, y_pred),
                "Fit Time (s)": fit_time }
    log_experiment("Lasso Regression", default_params, metrics) # RFE irrelevant
...
def fit_transformation(df, target_label, baseline_ll_col, group_id_col, trans=False):
    ...
    # Set target to NaN where transformation is not possible
...
def truncate(df, group_id_col, kfold):
    ...
    # --- Handle Integer K-Fold Case ---
        ...
        if ndel != 0:
            ...
            df = df[~df[group_id_col].isin(ids_to_remove)].copy() # Use .copy()
            groups_ids = df[group_id_col].unique() # Update group IDs
...
# --- Cross-Validation / Validation Set Function (Updated Signature) ---
...
def evaluate_model(df, target_label, feature_cols, group_id_col, model_name, kfold_value,
                   ...
                   n_features_rfe=None): # Added RFE parameter
    ...
    # Add RFE argument only if the model is RFR
    if model_name == 'RFR':
        model_args['n_features_to_select'] = n_features_rfe
    ...
    if not validation_set:
        ...
        df_cv["pred"] = np.nan
        fold_num = 0
        # Iterate through the group IDs in batches
        for low_i_idx in range(0, len(groups_ids), test_batch_size):
            ...
            # Get train/test data splits for this fold
            ...
            # Update model args for the current fold
            model_args['df_test'] = df_test_fold
            model_args['df_train'] = df_train_fold

            # Call the selected model function with appropriate args
            y_pred_fold, oob_fold, f_imp_fold = apply_func(**model_args)
            ...
            if y_pred_fold is not None:
                # Assign predictions back to the correct rows in df_cv
        ...
        df_out = df_cv # The output dataframe is the one used for CV
        ...
        df_train = df_out # Training data is the input dataframe
        ...
        if trans: required_val_cols.append(BASELINE_LL_COL)
        ...
        # Apply transformation to validation set if needed
        if trans:
        ...
        # Drop rows with NaN target label *after* potential transformation
        ...
        # Update model args for validation run
        model_args['df_test'] = df_test
        model_args['df_train'] = df_train

        # Call the selected model function
        y_pred_val, oob_val, f_imp_val = apply_func(**model_args)
        ...
        df_out = df_test # Output dataframe is the test set with predictions

    # --- Calculate final scores (based on df_out) ---
    print("\nCalculating final scores...")
    # Check if 'pred' column exists and has non-NaN values
    ...
    if df_scored.empty:
        ...
        # Calculate ranks and correlations per group
...
# --- Result Formatting (print_and_index_results handles mapped importances) ---
...
def print_and_index_results(res_dict, features):
    ...
    # --- Feature Importances ---
    # 'f_importance' should now be the mapped array (length = original features)
    ...
    if mean_importances is not None:
        ...
        num_expected_features = len(features) # Use the input feature list length
        if len(mean_importances) == num_expected_features:
            # Create pairs of (feature_name, importance)
            # Use the global FEATURE_COLS which excludes 'group_id'
            feature_importance_pairs = sorted(zip(FEATURE_COLS, mean_importances), key=lambda x: x[1], reverse=True)
            ...
            for feature, imp_val in feature_importance_pairs:
                 ...
                 # Only print non-zero importances for clarity, especially if RFE was used
                 if imp_val > 1e-6: # Use a small threshold to account for float precision
                     print(f"  {feature}: {imp_val:.4f}")
            print("  (Features with ~0 importance are not shown)")
        else:
...
# --- Function to process kfold argument (unchanged) ---
...
# --- Main Execution Block (Added RFE Arg) ---

if __name__ == '__main__':
    # --- Argument Parsing ---
    parser = argparse.ArgumentParser(description='Run ML algorithm using default parameters on TBR features, with optional RFE for RFR.')
    ...
    # Add argument for RFE feature count
    parser.add_argument('--n_features_rfe', type=int, default=None, help='Number of features to select using RFE (only applies if model is RFR). Default: None (RFE disabled).')
    ...
    if args.model == 'RFR':
        if args.n_features_rfe is not None:
            print(f"RFE Enabled for RFR: Selecting top {args.n_features_rfe} features.")
            if args.n_features_rfe <= 0 or args.n_features_rfe > len(FEATURE_COLS):
                 print(f"Error: --n_features_rfe must be between 1 and {len(FEATURE_COLS)}.")
                 sys.exit(1)
        else:
            print("RFE Disabled for RFR.")
    else:
        if args.n_features_rfe is not None:
            print("Warning: --n_features_rfe is specified but model is not RFR. RFE will not be applied.")
            args.n_features_rfe = None # Ensure it's None if model isn't RFR
    ...
    # Drop rows where the target label is NaN
    ...
    start_time = time.time()
    # Pass the n_features_rfe argument to evaluate_model
    res_dict, df_with_preds = evaluate_model(
        ...
        random_choice=False,
        scale_score=True,
        n_features_rfe=args.n_features_rfe # Pass the RFE count
    )
    ...
    print("\n--- Aggregated Results ---")
    # Pass FEATURE_COLS for correct feature importance naming
