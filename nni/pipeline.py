...
def generateStartingTree(dataset_path):
    # Executes all processes for a given dataset sequentially.
    print("START generateStartingTree:", dataset_path, "at", datetime.datetime.now())
    if os.path.exists(dataset_path):
        executePhyml(dataset_path)
    print("DONE generateStartingTree:", dataset_path, "at", datetime.datetime.now())
...
def main(training_folders):
    ...
    # Use multiprocessing to run generateStartingTree in parallel
    with ProcessPoolExecutor(max_workers=min(8, os.cpu_count()-2)) as executor: #adjust max workers accordingly
        executor.map(generateStartingTree, dataset_paths)
